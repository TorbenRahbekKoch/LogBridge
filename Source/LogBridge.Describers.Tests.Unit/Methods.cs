...
namespace SoftwarePassion.LogBridge.Describers.Tests.Unit
{
    public static class Methods
    {
        ...
        [MethodImpl(MethodImplOptions.NoOptimization)]
        public static MethodDescriptor Method18(TimeSpan value)
        {
            return Describe.MethodAndParameters(value);
        }

        [MethodImpl(MethodImplOptions.NoOptimization)]
        public static MethodDescriptor LambdaMethod(int value)
        {
            Func<MethodDescriptor> describer = Describe.CreateDescriber(value);
            return RunLambda(describer);
        }

        [MethodImpl(MethodImplOptions.NoOptimization)]
        private static MethodDescriptor RunLambda(Func<MethodDescriptor> describer)
        {
            return describer();
        }
    }
